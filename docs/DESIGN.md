...
| データベース | SQLite | better-sqlite3 9.x | 同期APIのためasync/awaitと組み合わせて非同期制御を明示的に学べる。ファイル1本で永続化 |
...
| 設計確定日+2 | 仕様曖昧箇所10点を一括解消 | 実装中の迷いをなくすため設計レビューを実施。JWT有効期限15分・updated_atトリガー追加・GET /api/notesに上限100件・WebSocket認証フロー追加・送信者除外方針・フォルダCASCADE削除・store管理対象明確化・S03初期表示定義・Typed Arrays矛盾除去・vite.config.js配置確定 |
